...
class Infer:
    def __init__(self):
        ...
        self.tracker_model = os.path.join(d,self.config['models']['tracker'])
        self.detection_model = self.config['models']['detection']
        self.tracker = DeepOcSort(
            reid_weights=Path(self.tracker_model), # which ReID model to use
            device='cpu',
            half=False,
            fp16=False,
            per_class=False
        )
        self.ModelPath = os.path.join(d, self.detection_model, 'best.pt')
